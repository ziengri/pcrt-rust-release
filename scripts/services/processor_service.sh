...
usage() { echo "Usage: processor_service.sh install --project-root PATH --env FILE [--binary FILE] | uninstall"; }
...
install_service() {
  ...
  binary="${binary:-$project_root/target/release/pcrt-processor}"
  [[ -f "$env_file" ]] || die "Env file not found: $env_file"
  [[ -x "$binary" ]] || die "Processor binary is not executable: $binary"
  cat >"$UNIT_PATH" <<EOF
[Unit]
